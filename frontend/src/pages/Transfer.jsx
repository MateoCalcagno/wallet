...
function Transfer() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!identifier.trim()) { setError('Ingresá un CBU o alias'); return }
    if (!esCBU && !esAlias) { setError('CBU inválido (22 dígitos) o alias inválido (6 a 20 caracteres)'); return }
    if (!amount || amount <= 0) { setError('Ingresá un monto válido'); return }
    if (amount > MAX_TRANSFER) { setError('El monto máximo por transferencia es $100.000'); return }
